...
function loadLogo() {
    ...
    image.onload = function () {
        ...
        const imageData = ctx.getImageData(0, 0, config.logoSize,
            config.logoSize);
...
function updatePhysics() {
    ...
    animationCount--;
    const radiusSquared = config.distortionRadius * config.
        distortionRadius;
...
particle.velocityY += force * Math.sin(angle) * config.forceStrength * forceMultiplier;
        ...
        const distFromOrigin = Math.sqrt(offsetX * offsetX + offsetY *
            offsetY);
...
function render() {
    ...
    const resolutionLoc = gl.getUniformLocation(program,
        "u_resolution");
